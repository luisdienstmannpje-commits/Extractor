...
class ConsistenciaSalario(LegalRule):
    ...
    def aplicar(self, contexto):
        ...
        v = _parse(s)
        if v is None: self._alerta(contexto, f"Salario '{s}' nao interpretavel.", nivel="AVISO")
        elif v <= 0: self._alerta(contexto, f"Salario invalido: R$ {v:.2f}.", nivel="ERRO")
        elif v < _MIN: self._alerta(contexto, f"Salario R$ {v:,.2f} abaixo do minimo R$ {_MIN:,.2f}.", nivel="AVISO")
        elif v > _MAX: self._alerta(contexto, f"Salario R$ {v:,.2f} acima de R$ {_MAX:,.0f}. Verificar extracao.", nivel="AVISO")
