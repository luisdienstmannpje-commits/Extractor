...
| Extrator: cache por contexto e **petição inicial** (`cache_context=peticao_inicial`; memorial com trechos; Excel `Pedidos_Inicial_*`; UI sem PJC) | `cache_key.py`, `memorial_pedidos.py`, `excel_exporter.py`, `api/routers/exports.py`, `POST /api/extract`, `POST /upload` — `backend/docs/PIPELINE.md` |
...
| Export Excel + `.pjc` (PJeCalc 2.14.0); Excel de **pedidos** na petição inicial | `services/excel_exporter.py`, `api/routers/exports.py`, `services/pjc_exporter.py` |
