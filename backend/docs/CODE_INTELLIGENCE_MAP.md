...
- **Layout SaaS** (Tailwind CSS em `index.html`):
  - **Sidebar fixa** (240px): logo, navegação Extrator / Laboratório, versão; troca de view via `data-nav-view` e script inline (`_showView`); preferência salva em `localStorage`.
  - **Header fixo**: título e subtítulo dinâmicos da view atual, créditos, avatar; backdrop blur.
  - **Quatro views**: `#view-extrator` (split-view: upload-state/viewer-state + KPIs + resultados com acordeões), `#view-lab` (Laboratório **8 uploads**, regras preditivas, Conclusão da Tríade de Ouro Expandida), `#view-historico` (Meus Processos), `#view-estatisticas` (Dashboard de IA: KPIs, últimas regras, top verbas). Apenas uma visível por vez.
