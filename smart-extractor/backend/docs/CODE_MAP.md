...
|---------|-------------------|
| `main.py` | API FastAPI; **`POST /upload`** aceita `files: List[UploadFile]` — PDF, Word, Excel (.xlsx/.xls), PJC, XML, JPG/PNG (`_EXT_UPLOAD`); roteia 1 PDF → `process_lawsuit_pdf`, dossiê → `process_lawsuit_dossie`; jobs; WebSocket; export; lab; stats; timeout 5 min. |
...
| `extraction_engine.py` | **Raio-X:** `enriquecer_para_raiox(dados)` — ESTRUTURAL_KEYS (Bloco 1) inclui prescricao_quinquenal; CONTRATUAL_KEYS (Bloco 2) sem prescricao. Categorias: Estrutural, Contratual, Condenação; Dicas Lab. Frontend: `render.js` BLOCO1_KEYS = ordem Bloco 1 (identificação + prescricao_quinquenal). |
...
| Função | Descrição |
|--------|-----------|
| `_classificar_tier_decisao(filename, texto?)` | **Só Título Executivo (Card 3).** Prioridade nome: 1grau/ATOrd→1GRAU, 2grau/ROT→TRT. Texto só se nome genérico; ignora 1000 chars; no texto só Recurso Ordinário/Relator:/Acórdão. Não afeta extração (sentença/acórdão). |
| `_extrair_data_documento(texto)` | **Só Título Executivo (data do doc).** Remove linha "Data da Autuação" antes de regex; prioriza final do doc. Na extração principal essa linha é mantida e mapeada para data_ajuizamento. |
...
| `js/render.js` | `renderResultado(dados)` — Raio-X + corpo (Parecer, Parâmetros, Verbas). Tabela "Dados extraídos do PDF" removida; identificação só no Raio-X. `#numero-processo` (hidden) para nome do Excel. |
...
| Título Executivo Complexo | `learning_engine.py` → `_classificar_tier_decisao`, `_extrair_data_documento` (remover Data da Autuação só aqui), `_extrair_titulo_executivo_multiplos`; `main.py`; `lab.js`. **Não confundir com extração:** na extração (processor + ai_client) o cabeçalho PJe é preservado e Data da Autuação vira data_ajuizamento. |
