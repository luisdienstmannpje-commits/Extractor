...
- **Pipeline:** 10 passos em `workers/processor.py`; não alterar ordem/contratos (ver também passo **7c** em `PIPELINE.md`: `verba_page_anchor.anchor_verbas_to_pages` preenche `pagina_origem`; cascata substring → prefixo/sufixo → compact → fuzzy guardado). Frontend: `AnalysisReport` + `PdfViewer` com `onNavegar`; texto *Fonte não ancorada* quando há trecho sem página. **Rastreabilidade por campo:** `dados_finais["fontes_extracao"]` (modelo `FonteExtracao`) é preenchido por `_coletar_fontes_extracao` para cabeçalho, verbas, teses, quadro comparativo e `alertas_juridicos[i]`; usar isso na UI para botão `🎯 pág. N` também no bloco de alertas. Para alertas inferidos, `confianca` (0–1) sinaliza força do vínculo e a UI exibe badge `conf. alta/média/baixa` + filtro rápido (`Todos` / `Somente conf. alta`) com persistência em `localStorage` (`analysis_alerta_filtro`), além de `Expandir/Recolher` por alerta com persistência em `analysis_alertas_expandidos` e controles globais `Expandir todos`/`Recolher todos` no subconjunto visível. **Cache multi-contexto:** `pdf_cache_storage_key` — `cache_context=auto` (padrão) mantém chave só SHA do ficheiro; outros valores (ex. `peticao_inicial`, `contestacao`) isolam entradas na mesma coluna SQLite. **Petição:** `POST /extract`, `/api/extract` e **`POST /upload`** com Form `cache_context=peticao_inicial` (regras em `extractor._should_run_process_lawsuit_pdf_upload`); memorial + export Excel dedicado (`excel_exporter` + `exports.py`); **sem** botão PJC na UI para `doc_type`/`_meta_doc_type` petição. **Contestação:** `cache_context=contestacao` — `_pipeline_contestacao`, `teses_defesa`, Excel `Contestacao_*.xlsx`, UI quadro de teses; sem PJC nem verbas deferidas. Ver `PIPELINE.md` § fluxos petição e contestação. **Dossiê (multi-upload):** `POST /upload` com `files: List[UploadFile]` — PDF, Word, Excel (.xlsx/.xls), PJC, XML, imagens (JPG/PNG). Um PDF (auto) → `process_lawsuit_pdf`; múltiplos ou não-PDF (auto) → `process_lawsuit_dossie`. Com **≥3** ficheiros, segunda passagem IA pode preencher **`quadro_comparativo`** (pedido × defesa × decisão); falha da 2ª IA não bloqueia o restante. Leitura: Excel → openpyxl (texto); imagens → Gemini multimodal (`ai_client.extrair_texto_ou_descricao_imagem`). Cache do dossiê por hash do conjunto. UI: Raio-X + tabela comparativa quando houver linhas.
- **Regras jurídicas:** só em `legal_engine/`; uma regra por arquivo; `LegalRule._canonizar_verba` para canonização. **Regras dinâmicas shadow (KB):** `executar_shadow_pipeline` preenche `dados_finais["shadow_logs"]` e `memoria_*.json`; não viram alertas de UI; multi-tenant via `KnowledgeBase(tenant_id)` em `carregar_regras_shadow`.
- **Guardrails:** `_filtrar_*` em `learning_engine.py` após enriquecimento; não remover nem mover.
- **Frontend:** SPA React (Vite + TypeScript + Tailwind); componentes em frontend/src/; rotas em App.tsx. `useAnalyze` já aceita `partial_update` (estado `partialData`) e as páginas `Extractor`/`Laboratory` exibem `Terminal Live Log` + `AnalysisReport` em modo loading (skeleton). **Backend:** `POST /upload` + `/ws/{job_id}` — o worker chama `push_ws_partial` em etapas (IA+dedup, ancoragem, motor+fontes, parecer; petição/contestação/dossiê conforme fluxo). `/api/extract` síncrono não emite parciais. `/ws/lab-pipeline` mantém heartbeat (Lab ainda síncrono).
...
3. **Multi-âncora (PDF):** `extract_sentence_from_pdf` mantém o bloco decisório atual; em seguida pode anexar páginas **fora** desse recorte (modificativa/liquidação vs. parâmetros/acordo/ata), com tetos — ver `CODE_MAP` (`sentence_finder.py`) e `ALTERACOES_VS_DOCUMENTACAO.md` § 0.6.
4. **Diagnóstico do texto até à IA:** `DEBUG_PIPELINE=1` (e opcionalmente `DEBUG_PIPELINE_DUMP=1`) — `services/pipeline_debug.py` + logs em `processor` e `ai_client._call_model`; quando há truncagem cirúrgica, `[DEBUG-PIPELINE][truncate]` expõe geometria (`disp_pos`, `start2`, `end2`, fallback verbas). Ver `PIPELINE.md` (secção debug) e `ALTERACOES_VS_DOCUMENTACAO.md` § 0.7.
...
2. `api/routers/exports.py` (`POST /api/export/excel`, `/api/export/pjc`); `services/pjc_exporter.py`, `pjc_template_patcher.py`, `excel_exporter.py`, `memoria_calculo/generator.py`. Petição inicial: Excel dedicado (`Pedidos_Inicial_*.xlsx`), PJC oculto na UI do extrator.
...
- `legal_validator.py` é wrapper; não reintroduza lógica ali. `validar_dados_completo` deve repassar `memorial_juridico` como lista de dicts (como o engine), não string — `gerar_explicacoes` depende disso no fluxo de sentença.
