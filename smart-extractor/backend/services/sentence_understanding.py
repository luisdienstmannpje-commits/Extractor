...
def _extrair_nome_verba(texto: str) -> Optional[str]:
    ...
    Busca substring das chaves do MAPA_NORMALIZACAO no texto pré-processado.

    Fix v5.3: usa _pre_processar() (lowercase + normalização de espaços)
    em vez de normalizar() — as chaves do mapa já têm acentos, então a
    busca por substring funciona corretamente após apenas lowercase + strip.

    Exemplo:
      texto: "Férias proporcionais deferidas ao reclamante."
      _pre_processar → "férias proporcionais deferidas ao reclamante."
      padrao "férias proporcionais" → match ✅
    """
    from services.normalizer import MAPA_NORMALIZACAO, _pre_processar

    texto_proc = _pre_processar(texto)

    melhor = None
    maior_len = 0

    for padrao in MAPA_NORMALIZACAO.keys():
        if padrao in texto_proc and len(padrao) > maior_len:
            melhor = padrao
